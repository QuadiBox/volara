...
const Page = () => {
    ...
    const filteredList = requests?.filter((elem, idx) => elem?.status === 'completed').filter((elem, idx) => idx <= currNum);
    ...
    useEffect(() => {
        ...
        const fetchRequests = onSnapshotWithoutCondition(
            ...
            (documents) => {
                ...
                setTotalNum(documents?.filter((elem) => elem?.status === 'completed')?.length);
            }
        )
