...
const Page = () => {
    ...
    const filteredList = requests?.filter((elem, idx) => elem?.status === 'active').filter((elem, idx) => idx <= currNum && elem?.locationData?.country !== undefined && elem?.locationData?.country !== null);
    ...
    useEffect(() => {
        ...
        const fetchRequests = onSnapshotWithoutCondition(
            'requests',
            (documents) => {
                const firstFilter = documents?.filter((elem) => elem?.status === 'active' && elem?.locationData?.country !== undefined && elem?.locationData?.country !== null)
    ...
    return (
        <div className='dashmainCntn special isactive'>
            <h1>All Tasks / Active</h1>
