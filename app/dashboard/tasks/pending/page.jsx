...
const page = () => {
    ...
    const filteredList = requests?.filter((elem, idx) => elem?.status === 'pending').filter((elem, idx) => idx <= currNum);
    ...
    useEffect(() => {
        ...
        const fetchRequests = onSnapshotWithoutCondition(
            ...
            (documents) => {
                ...
                setTotalNum(documents?.filter((elem) => elem?.status === 'pending')?.length);
            }
        )
