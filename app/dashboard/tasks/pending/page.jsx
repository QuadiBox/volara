...
const Page = () => {
    ...
    const filteredList = requests?.filter((elem, idx) => elem?.status === 'pending').filter((elem, idx) => idx <= currNum && elem?.locationData?.country !== undefined && elem?.locationData?.country !== null);
    ...
    useEffect(() => {
        ...
        const fetchRequests = onSnapshotWithoutCondition(
            'requests',
            (documents) => {
                const firstFilter = documents?.filter((elem) => elem?.status === 'pending' && elem?.locationData?.country !== undefined && elem?.locationData?.country !== null)
    ...
    return (
        <div className='dashmainCntn special ispending'>
            <h1>All Tasks / Pending</h1>
